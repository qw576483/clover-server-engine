...
//     "被夹住时反向那份被裁掉"的振荡来源。
//
// 收敛由**逐 tick 的松弛**负责（每 tick 一轮），tick 内不再迭代。
//
// # 边界（什么不在这里）
//
//   - **不是 MMO 空间能力**：与 `pkg/domain/mmo/collide` 无 import 关系。本包只有"圆形群体
//     分离"，不做格子查询、不做宽相、不做射线、不做寻路。
//   - **不是浮点三维几何**：坐标 / 半径 / 额度一律是调用方自定的**定点整数**（如 1/1000 格）。全整数运算 ⇒ 同输入同输出，可直接进服务器权威模拟与离线断言，不引入
...
// # 复杂度
//
// `Resolve` 是 O(n²) 全对枚举。它是给"一屏内的近战拥挤"用的，**不含**
...
//
// 静态障碍与带 ignore-pushback 标记的实体都用它，因此"一推不动"在质量分摊公式里自然成立，不需要
// 额外的 if 分支去特判方向。
...
type Body struct {
	...
	Radius int64
	// Mass 是碰撞质量：> 0 = 可推动（对 < 1 也按 1 处理），≤ 0 = **不可推动**
	// （静态障碍、塔、带 ignore-pushback 标记的实体）。质量只用于分摊重叠量：
	// 一方越重，被推开的那一方承担越多；不可推动的一方恒为 0 位移。
	Mass int64
	// Layer 是分层标识：只有 Layer 相同的两个刚体才会互相分离（调用方用它把空中层与
