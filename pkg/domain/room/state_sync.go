...
//
// 提炼自本工作区里一个**进程内状态同步房间**实现（迁移来源与逐条对照见交付报告），
// 三条语义在本实现里都保留：
//
...
//
// 与源实现的差异（写清楚，便于对照与后续演进）：
//   - 座位用**固定槽**：切片长度 = 座位数，下标即队伍号，删人不移动其它座位。
//     因此不存在源实现里的「座位号漂移」，也就不需要那套「座位一变就清空准备态」
//     的补偿逻辑（用固定槽把这条约定变成了结构上的不变量）。
//   - 观察者的「已下发版本」只在**推送成功之后**记录（源实现是先记版本再推，
//     推送失败不会补推，兜底因此大约只覆盖「注册但没被推过」这一种情况）。
...
func (k *StateSyncKernel) Watch(playerID string) (RoomList, error) {
	...
	list := k.listLocked()
	// 返回值就是首帧下发载体（对应源实现里「拉列表即回全量」），
	// 因此这里标记为已同步到当前版本；之后的版本由推送路径按成功与否记录。
