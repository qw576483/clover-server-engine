...
Clover 的 **Go 游戏服务端引擎**：进程编排、网关与鉴权、集群服务发现、数据持久化、事件总线、跨节点通信、MMO 世界、房间与对象系统 —— 开箱即用的服务端底座。
...
包约定：**对外包应配套单测与 `README.md`**（现状未全覆盖：部分自包含小包仍缺 README，属待补项）。已知缺陷与修复记录见 [`修复记录.md`](修复记录.md)（S 编号体系）。
