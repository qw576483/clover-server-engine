...
// 级联加载出的子对象必须有同步装配（回归）
//
// 守的历史缺陷：loadChildren 会按持久化的子对象 id 补建 *GameObject 并挂进 childPtrs，
// 但**从不给它接同步**（objstore.Repository 的 Create/Load 都会接，内部补建的路径漏了）。
// 症状：父对象写变动客户端立刻能看到，子对象（背包道具 / 容器里的卡 / 军团成员）
