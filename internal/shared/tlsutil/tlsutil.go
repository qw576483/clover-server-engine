...
//
// 单独成包：etcd、nats 各自的 TLSConfig 结构体是**刻意保留**的（为解依赖方向
// 而在本地定义、经适配器转换），但「读文件 → 建证书池 → 装配 tls.Config」这段逻辑
// 没有必要各写一份（两边各写一遍时校验细节与错误文案会不一致：
...
//   - **只配其一**：视为配置错误直接报错（漏配私钥会让 mTLS 静默失效、握手报错难以定位，
//     与 gateway.tls_cert/tls_key 的成对校验口径一致）；
//   - 两者都不配：跳过客户端认证（合法用法）；
